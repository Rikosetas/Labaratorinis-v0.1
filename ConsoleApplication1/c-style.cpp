...
void spausdintiRezultatus( const Studentas* studentai, int m, bool mediana )
{
    std::cout << "\n" << std::string( 70, '-' ) << "\n";
...
Studentas* ivestiRankiniu( int& m, int& n )
{
    ...
    m = 0;
    int capacity = 2;
    Studentas* studentai = new Studentas [ capacity ];
    ...
    while ( true )
    {
        ...
        {
            capacity *= 2;
            Studentas* naujas = new Studentas [ capacity ];

            for ( int i = 0; i < m; i++ )
            {
                naujas [ i ].vardas = studentai [ i ].vardas;
                naujas [ i ].pavarde = studentai [ i ].pavarde;
                naujas [ i ].egzaminas = studentai [ i ].egzaminas;
                naujas [ i ].n = studentai [ i ].n;
                naujas [ i ].nd = new int [ studentai [ i ].n ];

                for ( int j = 0; j < studentai [ i ].n; j++ )
                    naujas [ i ].nd [ j ] = studentai [ i ].nd [ j ];
        ...
        studentai [ m ].n = n;
        studentai [ m ].nd = new int [ n ];

        for ( int j = 0; j < n; j++ )
        {
            std::cout << "  ND" << ( j + 1 ) << " balas (1-10): ";
            while ( !skaitytiSveika( studentai [ m ].nd [ j ], 1, 10 ) ) {
                std::cout << "  Neteisinga reiksme (1-10): ";
            }
        }

        std::cout << "Egzamino balas (1-10): ";
        while ( !skaitytiSveika( studentai [ m ].egzaminas, 1, 10 ) ) {
            std::cout << "  Neteisinga reiksme (1-10): ";
        }
...
int main( )
{
    ...
    do {
        ...
        case 1:
        {
            Studentas* studentai = ivestiRankiniu( m, n );
